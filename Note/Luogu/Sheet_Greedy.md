...
首先我们假设安排的顺序是$p_1,p_2,...,p_n$

$p_i$等待时间为$\sum_{j=1}^i T_{p_j}$

那么总等待时间是
$$
W = 0 + T_{p_1} + (T_{p_1}+T_{p_2}) + ... + (T_{p_1}+T_{p_2}+...+T_{p_{n-1}})
$$
...
若$a_1 \leq a_2 \leq ...\leq a_n$且$b_1 \geq b_2 \geq ... \geq b_n$，则
$$
a_1b_n + a_2b_{n-1} + ... + a_nb_1 \leq a_1b_{p_1} + a_2b_{p_2} + ... + a_nb_{p_n} \leq a_1b_1 + a_2b_2 + ... + a_nb_n
$$
...
$$
(a_sb_j + a_tb_i) - (a_sb_i + a_tb_j) = a_s(b_j-b_i) + a_t(b_i-b_j) = (a_t-a_s)(b_i-b_k) \geq 0
$$
...
到$\{a\}$升序并且$\{b\}$降序的时候，无法再进行逆序对换，这时总和最小$\square$
...
所以第一步选择结束时间最早的时间是不会错过最优解的

* 最优子结构性质

我们按照上一个性质往下推，我们可以拥有一个较早结束的活动块$\chi$

这个活动块与前面的$X$等效，比它晚结束的具有相同活动个数的活动块$\gamma$并不会比它更优

可以发现这个算法每次都能做出局部最优解的同时不耽误全局最优解

事实上正确的贪心策略都要题目能够满足以上性质
...
首先思考一件事情：重量最小的两堆果子一定在第一次就被合并吗？

事实上不一定在第一次，但一定会在某一步被单独合并，并且是不劣的，下面证明：

$\circ\ $存在一个最优方案，使得重量最小的两堆是**兄弟节点**（即在某一步会被合并）

用调整法说明

假设某个最优方案中最小堆和次小堆$m_1$和$m_2$不是兄弟节点

设$m_1$兄弟是$n_1$，$m_2$兄弟是$n_2$
...
$\circ\ $考虑第一对$(a_1,a_2)$，要么减少$a_1$要么减少$a_2$

如果减少$a_1$,只能对$(a_1,a_2)$这一对有作用

而减少$a_2$可以对后续产生影响，并且不劣

$\circ\ $考虑中间某一对$(a_i,a_{i+1})$

假如减少$a_i$会使得左侧已经完善的关系被破坏
